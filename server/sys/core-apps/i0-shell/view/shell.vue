...
export default {
  ...
  methods: {
    ...
    fs_change_dir_handler(input){        
        ...
        const 
            go_back_wards     = input.arguments_string == '..' || input.arguments_string == '../',
            go_one_level_up_a = input.arguments_string.split('/').length >= 1 && cur_inp.indexOf('..') == -1 && input.arguments_string.split('/').pop() == "",
            go_one_level_up_b = input.arguments_string.split('/').length == 1 && input.arguments_string != '..',
